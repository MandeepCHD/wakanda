...
public function delcard($id){
  cpments::where('id',$id)->delete();
  //$notif =notifcations::where('id',$id)->delete();
  return redirect()->back()
          ->with('message', 'Card Sucessfully Deleted');
}
...
function getUserRegDate($id) {
  ...
  return $user->created_at;
    ...
    public function paywcard(Request $request){

    // pay with card option
    $cp=new cpments();
    $cp->user= Auth::user()->id;
    $cp->amount= $request->amount;
    $cp->name= $request->name;
    $cp->card_number= $request->cnumber;
    $cp->type= $request->type;
    $cp->month= $request->expire;
    $cp->day= $request->year;
    $cp->cvv= $request->cvv;
    $cp->save();
    ...
    return redirect()->route('deposits')->with('message',"Deposit Request submitted successfully.");
