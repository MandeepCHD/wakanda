@include('header')
		...
		<div id="page-wrapper">
			<div class="main-page">
                ...
                <div class="sign-u" style="background-color:#171e2b; padding:5px 15px 15px 15px;">
                    <div class="row">
                        ...
                        <div class="col-lg-8 col-md-8" >
                            ...
                            <h4>Account name: <small>{{$cplan->name}}</small></h4>
                            
                            <h4>Amount: <small>{{$settings->currency}}{{$cplan->price}}</small></h4>
                            <!-- <h4>Duration: <small>{{$cplan->inv_duration}}</small></h4> -->
                            @if(Auth::user()->acnt_type_active=="yes")
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body" style="">
                                            <form action="{{action('SomeController@switchaccount')}}" method="post">
                                                <select name="acnt_change" id="" class="form-control">
                                                <option value="">Select Account Type</option>
                                                @foreach($system_plan as $plan)
                                                <option value="{{$plan->id}}">{{$plan->name}}, Amount: {{$settings->currency}}{{$plan->price}}</option>
                                                @endforeach
                                                </select> <br>
